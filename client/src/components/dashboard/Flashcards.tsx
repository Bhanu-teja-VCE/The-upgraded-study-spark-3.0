    ...
    if (cards.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] text-center">
                <div className="w-24 h-24 bg-[#6366F1]/10 rounded-3xl border border-[#6366F1]/20 flex items-center justify-center mb-6 animate-pulse shadow-[0_0_40px_rgba(99, 102, 241,0.15)]">
                    <Layers className="w-12 h-12 text-[#6366F1]" />
                </div>
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogTrigger asChild>
                        <Button size="lg" className="bg-[#6366F1] text-black hover:bg-[#6366F1]/80 rounded-xl font-bold tracking-wide shadow-[0_0_20px_rgba(99, 102, 241,0.3)] transition-all hover:scale-105">
                            <Wand2 className="w-4 h-4 mr-2" />
                            GENERATE DECK
                        </Button>
                    </DialogTrigger>
                    <DialogContent className="bg-[#0A0A0F]/95 backdrop-blur-xl border-[#6366F1]/30 text-white rounded-2xl">
                        <DialogHeader>
                            <DialogTitle className="font-bold text-[#6366F1] text-xl">Generate Flashcards</DialogTitle>
                        </DialogHeader>
                        <div className="space-y-4 py-4">
                            <Textarea
                                placeholder="Paste your notes here (e.g., 'The mitochondria is the powerhouse of the cell...')"
                                className="min-h-[200px] bg-black/50 border-white/10 focus:border-[#6366F1] rounded-xl focus-visible:ring-0 text-white text-sm leading-relaxed"
                                value={inputText}
                                onChange={(e) => setInputText(e.target.value)}
                            />
                            <Button
                                onClick={handleGenerate}
                                className="w-full bg-[#6366F1] text-black hover:bg-[#6366F1]/80 rounded-xl font-bold"
    ...
    return (
        <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] relative overflow-hidden">
            ...
            <div className="w-full max-w-xl h-1.5 bg-white/10 mb-8 rounded-full overflow-hidden">
                <motion.div
                    className="h-full bg-[#6366F1] shadow-[0_0_10px_#6366F1]"
                    initial={{ width: 0 }}
                ...
                <AnimatePresence mode="wait" initial={false} custom={direction}>
                    ...
                    >
                        ...
                        <div className={cn(
                            ...
                            "bg-[#0A0A0F] border border-[#6366F1]/30 shadow-[0_0_30px_rgba(99, 102, 241,0.1)]",
                            "hover:border-[#6366F1]/60 transition-colors"
                        )}>
                            <div className="absolute top-0 right-0 w-32 h-32 bg-[#6366F1]/5 rounded-full blur-3xl -z-10" />
                            <span className="absolute top-6 left-6 text-[#6366F1] text-xs font-mono tracking-widest">QUESTION_0{currentIndex + 1}</span>
                        ...
                        <div className={cn(
                            ...
                            "bg-[#0D0D12] border border-[#6366F1] shadow-[0_0_50px_rgba(99, 102, 241,0.15)]"
                        )}>
                            <span className="absolute top-6 left-6 text-[#6366F1] text-xs font-mono tracking-widest">ANSWER</span>
                            ...
                            <div className="absolute bottom-0 left-0 right-0 h-16 border-t border-white/10 flex divide-x divide-white/10 bg-black/20">
                                ...
                                <button className="flex-1 hover:bg-[#6366F1]/10 text-[#6366F1] text-xs font-bold transition-colors">EASY</button>
                            </div>
                        </div>
                    </motion.div>
                </AnimatePresence>
            ...
            <div className="flex items-center gap-6 mt-10">
                <Button
                    ...
                    className="bg-black/50 border-[#6366F1]/30 text-[#6366F1] hover:bg-[#6366F1] hover:text-black rounded-xl w-14 h-14 backdrop-blur-sm transition-all shadow-[0_0_20px_rgba(99, 102, 241,0.1)] hover:shadow-[0_0_30px_rgba(99, 102, 241,0.3)] disabled:opacity-30 disabled:hover:shadow-none"
                >
                    <ArrowLeft className="w-6 h-6" />
                </Button>
                <span className="font-mono text-lg text-gray-400 font-bold">
                    <span className="text-[#6366F1]">{currentIndex + 1}</span> / {cards.length}
                </span>
                <Button
                    variant="outline"
                    size="icon"
