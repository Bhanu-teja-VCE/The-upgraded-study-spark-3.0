...
const AuthLayout = ({ children, title, subtitle }: { children: React.ReactNode; title: string; subtitle: string }) => {
  ...
  return (
    <div className="min-h-screen bg-[#050507] flex items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-[#00D9FF] selection:text-black">

      {/* 1. The Interactive Cursor Spotlight (Background) */}
      <div
        className="pointer-events-none fixed inset-0 z-0 transition-opacity duration-300"
        style={{
          background: `radial-gradient(600px circle at ${mousePosition.x}px ${mousePosition.y}px, rgba(0, 217, 255, 0.15), transparent 80%)`
        }}
      />
...
const InputField = ({ type, placeholder }: { type: string; placeholder: string }) => (
  <div className="group relative">
    <input
      type={type}
      placeholder=" "
      className="peer w-full p-4 bg-[#12121A] border border-white/5 rounded-xl text-white outline-none focus:border-[#00D9FF]/50 transition-all duration-300 placeholder-transparent z-10 relative"
    />
    <label className="absolute left-4 top-4 text-gray-500 text-sm transition-all duration-300 peer-placeholder-shown:text-base peer-placeholder-shown:top-4 peer-focus:-top-2.5 peer-focus:text-xs peer-focus:text-[#00D9FF] peer-focus:bg-[#0A0A0F] peer-focus:px-1 pointer-events-none z-20">
      {placeholder}
    </label>
    {/* Glow effect on input focus */}
    <div className="absolute inset-0 rounded-xl bg-[#00D9FF] opacity-0 peer-focus:opacity-5 blur-md transition-opacity duration-300 pointer-events-none" />
  </div>
);
    ...
    <AuthLayout title="Welcome Back" subtitle="Sign in to continue your streak on StudySpark">
      <div className="space-y-5">
        ...
        <div className="flex justify-end">
          <a href="#" className="text-xs text-gray-400 hover:text-[#00D9FF] transition-colors">
            Forgot Password?
          </a>
        </div>

        <a href="/dashboard" className="block">
          <button className="w-full py-4 bg-gradient-to-r from-[#00D9FF] to-[#0099FF] text-black font-bold rounded-xl hover:shadow-[0_0_40px_rgba(0,217,255,0.3)] transform transition-all duration-300 active:scale-95 relative overflow-hidden group">
        ...
        <p className="text-center text-gray-500 text-sm mt-6">
          ...
          <a href="/register" className="text-[#00D9FF] hover:text-[#00D9FF]/80 font-medium transition-colors">
            Sign up
          </a>
        </p>
      </div>
    ...
    <AuthLayout title="Create Account" subtitle="Join StudySpark and master your productivity">
      <div className="space-y-5">
        ...
        <a href="/dashboard" className="block mt-2">
          <button className="w-full py-4 bg-gradient-to-r from-[#8B5CF6] to-[#00D9FF] text-white font-bold rounded-xl hover:shadow-[0_0_40px_rgba(139,92,246,0.3)] transform transition-all duration-300 active:scale-95">
            Create Account
          </button>
        </a>
        ...
        <p className="text-center text-gray-500 text-sm mt-6">
          ...
          <a href="/login" className="text-[#00D9FF] hover:text-[#00D9FF]/80 font-medium transition-colors">
            Sign in
          </a>
        </p>
      </div>
