    ...
    return (
        ...
        >
            ...
            <motion.div variants={item} className="flex justify-between items-end">
                <div>
                    <h1 className="text-4xl font-bold bg-clip-text text-foreground mb-2">
                        Welcome back, {userName.split(' ')[0]}! 👋
                    </h1>
                    <p className="text-muted-foreground text-lg">
                        Let's make today productive. You have <span className="text-primary">4 tasks</span> due soon.
                    </p>
                </div>
                <div className="hidden md:block text-right">
                    <p className="text-sm text-muted-foreground font-mono">CURRENT SESSION</p>
                    <p className="text-2xl font-mono text-primary animate-pulse">00:00</p>
                </div>
            </motion.div>
                ...
                <motion.div variants={item} className="lg:col-span-2 space-y-8">
                    ...
                    <section>
                        ...
                        <div className="glass-card p-8 space-y-2 rounded-2xl text-center">
                            ...
                            <p className="text-sm text-muted-foreground">Tasks from your planner will appear here.</p>
                            <Link to="/dashboard/planner">
                                <Button variant="ghost" className="text-primary mt-2 underline">Go to Planner</Button>
                            </Link>
                        </div>
                    </section>
                </motion.div>
                    ...
                    <section className="h-full">
                        ...
                        <div className="glass-card p-6 h-full min-h-[400px] rounded-2xl relative overflow-hidden flex flex-col items-center justify-center text-center">
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-sm text-muted-foreground mb-6">Your study history will show up here.</p>

                                <Link to="/dashboard/notes">
                                    <Button className="bg-muted/50 hover:bg-muted text-foreground border border-border">
                                        Start Studying
                                    </Button>
                                </Link>
                            </div>
...
function StatCard({ icon: Icon, value, label, variant }: any) {
    const variants: any = {
        orange: { color: "text-orange-500", bg: "bg-orange-500/10", border: "border-orange-500/20" },
        blue: { color: "text-blue-400", bg: "bg-blue-400/10", border: "border-blue-400/20" },
        purple: { color: "text-purple-400", bg: "bg-purple-400/10", border: "border-purple-400/20" },
    ...
    return (
        <Card className={`glass-card border-none relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300`}>
            <div className={`absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity ${style.color}`}>
                <Icon className="w-16 h-16" />
            </div>
            <CardContent className="p-6">
                <div className={`w-10 h-10 rounded-lg ${style.bg} flex items-center justify-center mb-4 ${style.color} ring-1 ring-inset ${style.border}`}>
...
function ActionCard({ icon: Icon, title, desc, primary, href }: any) {
    const Content = (
        <div className={cn(
            "text-left p-6 rounded-2xl border transition-all duration-300 relative overflow-hidden group hover:shadow-lg h-full w-full block",
            primary
                ? "bg-gradient-to-br from-primary/20 to-[#8B5CF6]/20 border-primary/30 hover:border-primary"
                : "bg-card border-border hover:bg-muted/50 hover:border-border"
        )}>
            {primary && <div className="absolute inset-0 bg-primary/5 group-hover:bg-primary/10 transition-colors" />}
            ...
            <h3 className="text-lg font-bold text-foreground mb-1 group-hover:text-primary transition-all">{title}</h3>
            <p className={cn("text-sm", primary ? "text-primary" : "text-muted-foreground")}>{desc}</p>

            <ArrowUpRight className="absolute top-4 right-4 w-5 h-5 text-muted-foreground opacity-0 group-hover:opacity-100 group-hover:translate-x-1 -translate-y-1 transition-all" />
        </div>
    );
