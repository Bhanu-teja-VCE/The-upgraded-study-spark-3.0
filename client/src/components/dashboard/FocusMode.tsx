...
// --- STYLES & CONSTANTS ---
// Adapted for StudySpark: Cyan (#00D9FF), Navy (#0A0A0F), Red (#EF4444)
...
export default function FocusMode() {
    ...
    return (
        <div className="flex h-[calc(100vh-4rem)] bg-[#0A0A0F] text-white font-sans overflow-hidden rounded-3xl border border-white/5 mx-6 mb-6">
            {/* Internal Sidebar */}
            <nav className="w-64 bg-[#12121A] border-r border-[#00D9FF]/10 flex flex-col p-4 z-20">
                ...
                <div className="pt-4 border-t border-white/10 space-y-4">
                    <div className="px-2 text-xs">
                        <div className="flex justify-between mb-1">
                            ...
                            <span className="text-[#00D9FF]">{calculateStreak(data.sessions)} days</span>
                        </div>
                        <div className="w-full bg-white/5 h-1 rounded-full overflow-hidden">
                            <div className="h-full bg-[#00D9FF]" style={{ width: `${Math.min(100, (dayNumber / 90) * 100)}%` }}></div>
                        </div>
                    </div>
                </div>
            </nav>
            ...
            <main className="flex-1 flex flex-col overflow-y-auto relative bg-[#0A0A0F] scrollbar-thin scrollbar-thumb-gray-800">
                <header className="h-16 border-b border-white/5 flex items-center justify-between px-8 bg-[#0A0A0F]/80 backdrop-blur-md sticky top-0 z-10">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex flex-col">
                            <span className="text-sm font-medium max-w-[300px] truncate text-[#00D9FF]">
...
function NavItem({ active, onClick, icon, label }: any) {
    return (
        <button onClick={onClick} className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-lg transition-all ${active ? 'bg-[#00D9FF]/10 text-[#00D9FF] border border-[#00D9FF]/20' : 'text-gray-400 hover:text-white border border-transparent'}`}>
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <StatCard label="Deep Work Today" value={formatTime(totalToday)} subValue={`${Math.round((totalToday / 3600 / target) * 100)}% of target`} color="#00D9FF" />
                <StatCard label="Current Streak" value={`${calculateStreak(data.sessions)} Days`} subValue="Momentum" color="#00D9FF" />
                <StatCard label="Intensity Avg" value="8.4/10" subValue="Performance" color="#00D9FF" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-[#12121A] border border-white/5 rounded-2xl p-6">
                    <h3 className="text-lg font-bold mb-6 text-[#00D9FF]">Velocity Heatmap</h3>
                    <div className="h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={data.sessions.slice(-14).map((s: any) => ({ d: s.date.split('T')[0], h: s.duration / 3600 }))}>
                                ...
                                <Area type="monotone" dataKey="h" stroke="#00D9FF" fill="#00D9FF" fillOpacity={0.1} strokeWidth={3} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                <div className="bg-[#12121A] border border-white/5 rounded-2xl p-6 flex flex-col justify-center items-center">
                    <Trophy className="text-[#00D9FF] mb-4" size={48} />
...
function SessionView({ data, setData }: any) {
        ...
        return (
            <div className="absolute inset-0 bg-[#0A0A0F]/95 z-50 flex items-center justify-center p-6 backdrop-blur-sm">
                <div className="max-w-xl w-full bg-[#12121A] border border-white/10 p-10 rounded-3xl">
                    ...
                    <textarea value={output} onChange={(e) => setOutput(e.target.value)} placeholder="What did you build?" className="w-full bg-black border border-white/10 rounded-xl p-4 h-40 outline-none focus:border-[#00D9FF] text-white" />
                    <div className="flex gap-4 mt-8">
                        ...
                        <button onClick={handleFinish} disabled={analyzing} className="flex-1 py-4 bg-[#00D9FF] text-black font-black rounded-xl hover:scale-105 transition-transform">
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
            <div className={`text-9xl font-black transition-all font-mono ${isActive ? 'text-[#00D9FF] drop-shadow-[0_0_30px_#00D9FF44]' : 'text-white/10'}`}>
                {formatTime(seconds)}
            </div>
            <div className="mt-12 flex gap-8">
                {!isActive ? (
                    <button onClick={() => setIsActive(true)} className="w-24 h-24 rounded-full bg-[#00D9FF] text-black flex items-center justify-center hover:scale-110 shadow-[0_0_40px_rgba(0,217,255,0.3)] transition-all">
            ...
            {isActive && (
                <div className="mt-12 w-full max-w-sm space-y-4">
                    <div className="flex justify-between text-xs font-black uppercase text-gray-500">
                        ...
                        <span className="text-[#00D9FF] animate-pulse">FLOW STATE ENGAGED</span>
                    </div>
                    <input type="range" min="1" max="10" value={intensity} onChange={(e) => setIntensity(parseInt(e.target.value))} className="w-full accent-[#00D9FF]" />
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-12rem)]">
            <div className="flex-1 overflow-y-auto space-y-4 pr-4" ref={scrollRef}>
                {(data.chatHistory || []).map((msg: any, i: number) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] p-4 rounded-2xl ${msg.role === 'user' ? 'bg-[#12121A]' : 'bg-[#00D9FF]/5 border border-[#00D9FF]/20'}`}>
            ...
            <div className="mt-6 flex gap-3">
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} className="flex-1 bg-[#12121A] border border-white/5 rounded-2xl px-6 py-4 outline-none focus:border-[#00D9FF] text-white" placeholder="Consult Bhanu..." />
                <button onClick={sendMessage} className="w-14 h-14 bg-[#00D9FF] text-black rounded-2xl flex items-center justify-center hover:bg-[#00D9FF]/80 transition-colors"><Send size={24} /></button>
            </div>
        </div>
    );
}
            ...
            <div className="bg-[#12121A] border border-white/5 rounded-2xl p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
                    ...
                    <div className="p-6 bg-black border border-white/5 rounded-2xl">
                        ...
                        <div className="text-3xl font-black text-[#00D9FF] mt-2">64%</div>
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in fade-in items-center">
            <div className="relative w-64 h-80 bg-white/5 rounded-full mx-auto flex items-center justify-center border border-white/10 overflow-hidden">
                <User size={120} className={`${alignment > 50 ? 'text-[#00D9FF] drop-shadow-[0_0_20px_#00D9FF]' : 'text-gray-800'}`} />
            ...
            <div className="bg-[#12121A] border border-white/5 rounded-3xl p-8">
                ...
                <p className="text-gray-400 italic text-xl border-l-4 border-[#00D9FF] pl-6 py-4">"{data.profile.identity || "Identify yourself in the Command Center."}"</p>
...
function IdentityCheck({ label, passed }: any) {
    return (
        <div className={`flex justify-between items-center p-4 rounded-xl border ${passed ? 'border-[#00D9FF]/20 bg-[#00D9FF]/5' : 'border-white/5 bg-transparent opacity-30'}`}>
            <span className="text-sm font-bold">{label}</span>
            {passed ? <CheckCircle2 className="text-[#00D9FF]" size={18} /> : <XCircle className="text-gray-500" size={18} />}
        </div>
    );
}
...
function ProgressMaterializer({ data }: any) {
    ...
    return (
        <div className="bg-[#12121A] border border-white/5 rounded-3xl p-12 relative overflow-hidden h-[500px]">
            <div className="flex items-center gap-2 mb-12 text-[#00D9FF]">
            ...
            <div className="h-64 border-b border-white/10 flex items-end justify-center gap-1 overflow-hidden">
                {Array.from({ length: buildingCount }).map((_, i) => (
                    <div key={i} className="bg-[#00D9FF]/30 border-x border-t border-[#00D9FF] w-8 animate-in slide-in-from-bottom" style={{ height: `${Math.random() * 80 + 10}%`, animationDelay: `${i * 50}ms` }} />
                ))}
        ...
        <div className="fixed inset-0 bg-black/95 z-50 backdrop-blur-xl flex items-center justify-center p-6">
            <div className="max-w-xl w-full bg-[#12121A] border border-white/10 rounded-3xl p-10">
                ...
                <div className="relative">
                    <input value={goal} onChange={(e) => setGoal(e.target.value)} className="w-full bg-black border border-white/10 rounded-xl p-4 text-xl outline-none focus:border-[#00D9FF] text-white" placeholder="Enter Mission..." />
                    <button onClick={getCritique} className="absolute right-4 top-4 text-[#00D9FF]"><Sparkles size={20} /></button>
                </div>
                {critique && <div className="mt-4 p-4 bg-black rounded-xl text-xs italic text-gray-500 border border-[#00D9FF]/20">{critique}</div>}
                <button onClick={() => { onCommit({ goal, duration: 4, startTime: '09:00' }); onHide(); }} className="w-full bg-[#00D9FF] text-black font-black py-5 rounded-2xl mt-8 hover:scale-105 transition-all">I COMMIT TO THIS WORK</button>
            </div>
        </div>
    );
}
        ...
        <div className="fixed inset-0 bg-black flex items-center justify-center p-8 z-[100]">
            <div className="max-w-md w-full space-y-12">
                <div>
                    ...
                    <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Designation..." className="w-full bg-transparent border-b border-white/10 text-4xl outline-none focus:border-[#00D9FF] py-4 text-white" />
                </div>
                <div>
                    <h1 className="text-5xl font-black tracking-tighter text-white">WHAT DO YOU BECOME?</h1>
                    <input value={idnt} onChange={(e) => setIdnt(e.target.value)} placeholder="Target Identity..." className="w-full bg-transparent border-b border-white/10 text-2xl outline-none focus:border-[#00D9FF] py-4 text-white" />
                </div>
                <button onClick={() => onComplete({ name, identity: idnt })} className="bg-[#00D9FF] text-black px-12 py-4 rounded-full font-black text-xl hover:scale-110">ACTIVATE</button>
            </div>
        </div>
    );
}
